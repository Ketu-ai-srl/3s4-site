...
for (const latime of LATIMI) {
  test.describe('derularea, cu miscare, ' + latime, () => {
    ...
    for (const cale of PAGINI) {
      test(cale + ': ultima sectiune ajunge la varf cu subsolul SCOS in proba (pagina servita are subsol); martor: la 80svh nu ajunge', async ({ page }) => {
        await page.emulateMedia({ reducedMotion: 'no-preference' })
        await deschide(page, cale, latime)
        // subsolul (din layout) scos din flux IN PROBA: varful ultimei sectiuni nu mai poate veni din spatiul lui.
        // Pagina servita are subsol (layout inghetat, fara comutator): proba NU dovedeste pagina servita.
