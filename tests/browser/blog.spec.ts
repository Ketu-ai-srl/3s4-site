 ...
 *
 *   1. BUILD-UL REAL, cu registrul GOL (primul lot de articole se scrie separat): nu exista nicio
 *      pagina de blog (`/blog`, categoriile si orice articol dau 404), iar navigatia si harta de site
 *      nu duc spre blog.
 ...
 *      copiei, cu indexul `articole.json` regenerat prin aceeasi conducta. Doua controale cu esec
...
// ---------------------------------------------------------------------------------------------
// 1. Build-ul real: registrul gol, deci niciun drum spre blog
// ---------------------------------------------------------------------------------------------
...
test.describe('build-ul real, in starea registrului (' + (REGISTRU_GOL ? 'gol' : ARTICOLE.length + ' articole') + ')', () => {
  ...
  test('navigatia si harta de site duc spre blog numai cand exista articole', async ({ page, request }) => {
    ...
    if (REGISTRU_GOL) {
      ...
      for (const a of ARTICOLE) expect(harta).toContain('/blog/' + a.slug)
    }
  })
})
...
test.describe('copia cu articole sintetice', () => {
  ...
  test('articolul la 390: coloana de 358, caseta CTA cu padding 28 si butonul pe un rand, inruditele pe o coloana', async ({ page }) => {
    ...
    const buton = await page.locator('section[aria-labelledby="cta-titlu"] [data-tinta-lipsa="/inregistrare"]').boundingBox()
  ...
  test('raspunsul listarii (G-AI-02), cu declaratia pregatita in config/seo/blog.json', async ({ browser }) => {
    const fisier = JSON.parse(readFileSync(join(RADACINA, 'config', 'seo', 'blog.json'), 'utf8')) as {
      la_primul_articol: Record<string, DeclaratieRaspuns>
    }
    const m = await masoaraRaspunsul(browser, url('/blog'), fisier.la_primul_articol['/blog'])
