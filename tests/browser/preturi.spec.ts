 ...
 *
 * Cardul enterprise: ruta `/enterprise` nu exista in acest val, deci cardul e INERT (fara href, fara
 * focus). Plecarea spre dreapta si navigarea lui se pot proba abia cand ruta apare (S4-3b).
 *
...
test.describe('poarta si lumea pachetelor la 1440 x 900, cu miscare', () => {
  ...
  test('cardul enterprise e inert cat timp /enterprise lipseste: fara href, fara focus, fara efect', async ({ page }) => {
    await deschide(page)
  ...
  test('calculatorul: teaserul e inlocuit, focusul pe primul cursor, iar formula urmeaza cursoarele', async ({ page }) => {
    ...
    // trimite la Enterprise (inert cat timp ruta lipseste), nu recomanda Pro
