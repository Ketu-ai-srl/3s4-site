...
/**
 * Asteapta ca macheta pasului `k` sa-si termine secventa inainte de o masura axe. Macheta 3 insereaza
 * verificarile la 900 + 500 * n ms dupa activare, fiecare cu un fade de 250 ms, deci secventa se
 * termina la ~2,65 s - dupa pauza fixa de 1,2 s a probei. Masurat pe 26.09, baza singura: 5 din 10
 * reluari picau la pasul 3 pe `.verificare:nth-child(3) > .vText` (a treia apare la 1,9 s, adica in
 * mijlocul trecerii axe), iar in clipa masurii macheta avea 2 sau 3 verificari din 4. Axe citea
 * textul la opacitatea din mijlocul fade-ului; starea finala trecea in toate reluarile.
 * Numarul verificarilor se deriva din continut, nu se scrie de mana.
 */
async function machetaAsezata(page: Page, k: number): Promise<void> {
  const t0 = Date.now()
  if (k === 2) {
    await expect(machetaCard(page, 2)).toHaveAttribute('data-verificari', String(MACHETA_REGISTRU.verificari.length), { timeout: 10_000 })
  }
  // Animatiile finite (fade-uri, tranzitii) s-au terminat; cele infinite nu se asteapta.
  await expect
    .poll(() =>
      page.evaluate(
        (sel) =>
    ...
    .toBe(0)
  console.log('[macheta asezata pas ' + (k + 1) + '] dupa ' + (Date.now() - t0) + ' ms')
}
...
test.describe('atingerea, pe un ecran tactil de 1440 x 900', () => {
  ...
  test('atingerea pe un rand inchis il deschide si il tine deschis (la referinta inchidea tot)', async ({ page }) => {
    // PREMISA se citeste in clipa clicului, nu inainte: ceasul deschide randul 2 la 3,4 s dupa
    // activare, iar daca bataia cade intre verificarea "inchis" si atingere, atingerea il inchide -
    // comportament corect (comutare), dar alta proba. Pe CI (rularea 36200373839) testul a durat
    // 17 s fata de 5,3-5,7 s local (20 de reluari), iar randul a ramas "false" 10 s: deci atingerea
    // a fost primita (ceasul tacea) si a gasit randul deja deschis. Un ascultator de clic in faza de
    // captura, pe `window`, citeste starea inaintea handler-ului React; daca ceasul a batut primul,
    // scenariul se reia de la capat, cel mult de 3 ori.
    const randuri = machetaCard(page, 0).locator('button[aria-expanded]')
    let premisa = ''
    for (let incercare = 1; incercare <= 3; incercare++) {
      ...
      await page.evaluate((sel) => {
        const w = window as unknown as { __stareLaAtingere?: string }
        w.__stareLaAtingere = ''
        window.addEventListener(
          'click',
          (e) => {
            const b = (e.target as Element).closest(sel + ' [class*="card"] figure [data-rand]')
            if (b && w.__stareLaAtingere === '') w.__stareLaAtingere = b.getAttribute('data-rand') + ':' + b.getAttribute('aria-expanded')
          },
          true,
        )
      }, SECTIUNE)
      ...
      premisa = await page.evaluate(() => (window as unknown as { __stareLaAtingere?: string }).__stareLaAtingere ?? '')
      console.log('[atingere] incercarea ' + incercare + ': randul atins, in clipa clicului = ' + premisa)
      if (premisa === '1:false') break
    }
    // Clicul a ajuns chiar pe randul 2, inchis; altfel proba nu masoara ce spune titlul.
    expect(premisa).toBe('1:false')
