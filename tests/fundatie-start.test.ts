...
describe('pagina de start randata pe server', () => {
  ...
  it('martor POZITIV: tintele lipsa sunt randate inert, cu tinta asteptata in atribut', () => {
    const inerte = [...html.matchAll(/data-tinta-lipsa="([^"]+)"/g)].map((m) => m[1])
    const asteptate = toateLegaturileAcasa().filter((l) => l.href && l.ruta && !CAI_EXISTENTE.has(l.ruta))
    expect(inerte.length).toBeGreaterThan(10)
