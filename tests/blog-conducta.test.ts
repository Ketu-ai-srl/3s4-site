 ...
 * fabricii; nimic din ele nu ajunge in `src/`. Registrul real e GOL la felia asta (primul lot se scrie
 * separat), iar o proba de mai jos cere ca indexul `articole.json` sa fie exact indexul fisierelor .mdx
...
const CORP_IMPLICIT = [
  ...
  '## Ce spune legea',
  '',
  'Un paragraf cu o [legătură internă](/blog/alt-articol-de-proba), una [spre o pagină care nu există încă](/solutii/avocatura) și una [externă](https://legislatie.just.ro/Public/DetaliiDocument/1).',
  '',
...
describe('registrul blogului', () => {
  ...
  // Starea se deriva din registru, nu se scrie: azi e gol, iar primul lot de articole nu inroseste proba.
  it('caile blogului vin numai din registru: nimic in RUTE, iar in caile existente exact ce deschide registrul', () => {
    expect(RUTE.some((r) => r.cale.startsWith('/blog'))).toBe(false)
    expect([...CAI_EXISTENTE].filter((c) => c.startsWith('/blog')).sort()).toEqual([...caiArticole()].sort())
    expect(caiArticole().length === 0).toBe(ARTICOLE.length === 0)
  })
...
describe('pagina unui articol, cu 12 articole sintetice', () => {
  ...
  it('legaturile interne trec prin Tinta: o ruta care nu exista ramane inerta', () => {
    expect(html).toContain('data-tinta-lipsa="/solutii/avocatura"')
    expect(html).not.toContain('href="/solutii/avocatura"')
    expect(html).toContain('data-tinta-lipsa="/blog/alt-articol-de-proba"')
  })

  it('caseta CTA ia textul autorului; butonul spre /inregistrare e inert cat timp pagina lipseste', () => {
    expect(html).toContain('Textul casetei CTA scris de autor')
    expect(html).toContain(ARTICOL.cta.titlu)
    expect(html).toContain('data-tinta-lipsa="/inregistrare"')
  })
...
describe('declaratia GEO a blogului', () => {
  ...
  const fisier = JSON.parse(readFileSync(cale, 'utf8')) as {
    raspuns_autonom: Record<string, unknown>
    la_primul_articol: Record<string, unknown>
  }

  it('cat timp blogul nu e in RUTE, fisierul nu declara nicio ruta, iar cititorul nu gaseste abateri', () => {
    expect(existsSync(cale)).toBe(true)
    expect(Object.keys(fisier.raspuns_autonom).filter((k) => !k.startsWith('_'))).toEqual([])
    const r = citesteDeclaratiile(RADACINA, RUTE.map((x) => x.cale))
    expect(r.abateri).toEqual([])
  })

  it('declaratia pregatita pentru listare are forma ceruta de proba G-AI-02', () => {
    expect(Object.keys(fisier.la_primul_articol)).toEqual(['/blog'])
    for (const d of Object.values(fisier.la_primul_articol)) expect(formaDeclaratiei(d)).toBe(true)
  })
})
