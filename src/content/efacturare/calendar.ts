 ...
 * de raportare (ghidul ANAF D406). Prima perioada e luna verificarii (septembrie 2026, cu termen
 * pe 31 octombrie 2026).
 */
function termeneD406(): Termen[] {
  const lista: Termen[] = [];
  for (let i = 0; i < 12; i++) {
    const perioada = new Date(Date.UTC(2026, 8 + i, 1));
    const termen = new Date(Date.UTC(2026, 9 + i, 1));
    ...
    lista.push({
      ...
      descriere:
        "Termenul declarației informative D406 pentru contribuabilii cu raportare lunară: ultima zi calendaristică a lunii următoare perioadei de raportare.",
      sursa: "roSaft",
      alarmaZile: 7,
    });
  }
